...
// ML-DSA-65 type definitions
using MLDSA65Key = decltype(ML_DSA_65()
                           + Function<function::Keygen>()
                           + Block()
                           + BlockDim<128>());

using MLDSA65Sign = decltype(ML_DSA_65()
                            + Function<function::Sign>()
                            + Block()
                            + BlockDim<128>());

using MLDSA65Verify = decltype(ML_DSA_65()
...
public:
    MaxPerformanceBenchmark() {
        initializeGPU();
        printf("🚀 ML-DSA-65 GH200 Maximum Performance Benchmarking\n");
    ...
    std::vector<unsigned int> generateOptimalBatchSizes(size_t message_size) {
        size_t signature_padded = ((MLDSA65Sign::signature_size + 7) / 8) * 8;
        size_t single_op_memory = MLDSA65Key::public_key_size + MLDSA65Key::secret_key_size + 
                                 MLDSA65Key::workspace_size + MLDSA65Key::entropy_size +
...
bool ml_dsa_sign_safe(std::vector<uint8_t>& signatures, std::vector<uint8_t>& messages, size_t message_size,
    ...
    size_t memory_ops = (length_signature_padded + length_secret_key + message_size +
                        MLDSA65Sign::workspace_size + MLDSA65Sign::entropy_size);
...
void printFinalResults(const std::vector<BenchmarkResult>& results) {
    printf("╔══════════════════════════════════════════════════════════════════════════════════════════════════╗\n");
    printf("║                                🏆 GH200 ML-DSA-65 MAXIMUM PERFORMANCE RESULTS 🏆               ║\n");
    printf("╠══════════════════════════════════════════════════════════════════════════════════════════════════╣\n");
...
int main() {
    ...
    for (size_t i = 0; i < batch_sizes.size(); i++) {
        ...
        auto signature_padded = ((MLDSA65Sign::signature_size + 7) / 8) * 8;
    ...
    printf("🎉 ML-DSA-65 Benchmarking completed! Your GH200 has been pushed to its limits! 💪\n");
    return 0;
}
