...
// ML-DSA-87 type definitions
using MLDSA87Key = decltype(ML_DSA_87()
                           + Function<function::Keygen>()
                           + Block()
                           + BlockDim<128>());

using MLDSA87Sign = decltype(ML_DSA_87()
                            + Function<function::Sign>()
                            + Block()
                            + BlockDim<128>());

using MLDSA87Verify = decltype(ML_DSA_87()
...
{
    __shared__ uint8_t smem_ptr[MLDSA87Key::shared_memory_size];
    int block = blockIdx.x;
    auto public_key = public_keys + block * MLDSA87Key::public_key_size;
    auto secret_key = secret_keys + block * MLDSA87Key::secret_key_size;
    auto entropy    = randombytes + block * MLDSA87Key::entropy_size;
    auto work       = workspace   + block * MLDSA87Key::workspace_size;

    MLDSA87Key().execute(public_key, secret_key, entropy, work, smem_ptr);
}
...
{
    __shared__ uint8_t smem_ptr[MLDSA87Sign::shared_memory_size];
    int block = blockIdx.x;
    auto signature  = signatures  + block * ((MLDSA87Sign::signature_size + 7) / 8 * 8);
    auto message    = messages    + block * message_size;
    auto secret_key = secret_keys + block * MLDSA87Sign::secret_key_size;
    auto entropy    = randombytes + block * MLDSA87Sign::entropy_size;
    auto work       = workspace   + block * MLDSA87Sign::workspace_size;

    MLDSA87Sign().execute(signature, message, message_size, secret_key, entropy, work, smem_ptr);
}
...
{
    __shared__ uint8_t smem_ptr[MLDSA87Verify::shared_memory_size];
    int block = blockIdx.x;
    auto signature   = signatures  + block * ((MLDSA87Sign::signature_size + 7) / 8 * 8);
    auto message     = messages    + block * message_size;
    auto public_key  = public_keys + block * MLDSA87Verify::public_key_size;
    auto work        = workspace   + block * MLDSA87Verify::workspace_size;

    valids[block] = MLDSA87Verify().execute(message, message_size, signature, public_key, work, smem_ptr);
}
...
public:
    MaxPerformanceBenchmark() {
        initializeGPU();
        printf("🚀 ML-DSA-87 GH200 Maximum Performance Benchmarking\n");
    ...
    std::vector<unsigned int> generateOptimalBatchSizes(size_t message_size) {
        size_t signature_padded = ((MLDSA87Sign::signature_size + 7) / 8) * 8;
        size_t single_op_memory = MLDSA87Key::public_key_size + MLDSA87Key::secret_key_size + 
                                 MLDSA87Key::workspace_size + MLDSA87Key::entropy_size +
...
bool ml_dsa_keygen_safe(std::vector<uint8_t>& public_keys, std::vector<uint8_t>& secret_keys, 
    ...
    auto length_public_key = MLDSA87Key::public_key_size;
    auto length_secret_key = MLDSA87Key::secret_key_size;

    auto workspace = make_workspace<MLDSA87Key>(batch);
    auto randombytes = get_entropy<MLDSA87Key>(batch);
    ...
    auto kernel_func = [&]() {
        keygen_kernel<<<batch, MLDSA87Key::BlockDim>>>(d_public_key, d_secret_key, randombytes, workspace);
...
bool ml_dsa_sign_safe(std::vector<uint8_t>& signatures, std::vector<uint8_t>& messages, size_t message_size,
    ...
    auto length_secret_key = MLDSA87Sign::secret_key_size;
    auto length_signature = MLDSA87Sign::signature_size;
    auto length_signature_padded = ((length_signature + 7) / 8) * 8;

    auto workspace = make_workspace<MLDSA87Sign>(batch);
    auto randombytes = get_entropy<MLDSA87Sign>(batch);
    ...
    size_t memory_ops = (length_signature_padded + length_secret_key + message_size +
                        MLDSA87Sign::workspace_size + MLDSA87Sign::entropy_size);

    auto kernel_func = [&]() {
        sign_kernel<<<batch, MLDSA87Sign::BlockDim>>>(d_signatures, d_messages, message_size, d_secret_keys, randombytes, workspace);
        CUDA_CHECK(cudaGetLastError());
    };
...
bool ml_dsa_verify_safe(std::vector<uint8_t>& is_valids, const std::vector<uint8_t>& signatures, 
    ...
    auto workspace = make_workspace<MLDSA87Verify>(batch);
    auto length_signature = MLDSA87Verify::signature_size;
    auto length_signature_padded = ((length_signature + 7) / 8) * 8;
    auto length_public_key = MLDSA87Verify::public_key_size;
    ...
    size_t memory_ops = (length_signature_padded + length_public_key + message_size + sizeof(bool) +
                        MLDSA87Verify::workspace_size);

    auto kernel_func = [&]() {
        verify_kernel<<<batch, MLDSA87Verify::BlockDim>>>(d_valids, d_signatures, d_messages, message_size, d_public_keys, workspace);
        CUDA_CHECK(cudaGetLastError());
    };
...
void printFinalResults(const std::vector<BenchmarkResult>& results) {
    printf("╔══════════════════════════════════════════════════════════════════════════════════════════════════╗\n");
    printf("║                                🏆 GH200 ML-DSA-87 MAXIMUM PERFORMANCE RESULTS 🏆               ║\n");
    printf("╠══════════════════════════════════════════════════════════════════════════════════════════════════╣\n");
...
// **ESSENTIAL: Main function - this was missing or corrupted**
int main() {
    MaxPerformanceBenchmark benchmark;
    ...
    for (size_t i = 0; i < batch_sizes.size(); i++) {
        ...
        auto signature_padded = ((MLDSA87Sign::signature_size + 7) / 8) * 8;
        
        std::vector<uint8_t> public_keys(MLDSA87Key::public_key_size * batch);
        std::vector<uint8_t> secret_keys(MLDSA87Key::secret_key_size * batch);
        std::vector<uint8_t> signatures(signature_padded * batch);
        std::vector<uint8_t> messages(message_size * batch);
        std::vector<uint8_t> is_valids(batch);
    ...
    printf("🎉 ML-DSA-87 Benchmarking completed! Your GH200 has been pushed to its limits! 💪\n");
    return 0;
}
